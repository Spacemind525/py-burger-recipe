...
Create the `BurgerRecipe` class. Its `__init__` method should accept and save the number of ingredients 
...
- the `__set__` method, which sets the attribute value 
- (**note** that there should be no validation implemented, 
...
- the `validate` method, which checks the value type, and if the type is incorrect,
- it must raise the `TypeError` exception with the `Quantity should be integer.` message.
Then it should check whether the value is within `min_value` and `max_value`.
- If it’s not, it should raise the `ValueError` with the `Quantity should not be less than {self.min_value} and greater than {self.max_value}.` message.
...
- the `validate` method, which accepts the `value`
- and checks if it’s one of the provided options and,
- if not — raises the `ValueError` with the
- `Expected {value} to be one of {self.options}.` message.
